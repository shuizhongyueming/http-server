 ...
 * @typedef {import("http-proxy").ServerOptions} ProxyOptions
 */

 /**
  * @typedef {Object} LogError The error object that will be passed to the proxy cache log function.
  * @property {number} status The HTTP status code.
  * @property {string} message The error message.
  */

/**
 * @typedef {(req: IncomingMessage, res: ServerResponse, err: LogError) => void} LogFn The function that will be used for logging.
 * @typedef {(err: LogError, proxyRes: ServerResponse, req: IncomingMessage, res: ServerResponse, cacheFilePath: string) => void} ProxyCacheLogFn The function that will be used for logging proxy cache requests.
 * @typedef {(proxyReq: ClientRequest, req: IncomingMessage) => void} ProxyLogFn The function that will be used for logging proxy requests.
 */
 ...
 * @property {string | Object} [mimetypes] Custom mime types.
 ...
 * @property {ProxyOptions} [proxyOptions] The options to pass to the `http-proxy` module.
 * @property {string} [proxyCache] The path to cache proxied responses to.
 * @property {ProxyCacheLogFn} [proxyCacheLogFn] The function that will be used for logging proxy cache requests.
 * @property {string} [headerKeyForProxyUrl] The header key to use for the proxy URL.
 * @property {ProxyLogFn} [proxyLogFn] The function that will be used for logging proxy requests.
 */
  ...
  */
  constructor(options) {
    ...
    if (typeof options.proxy === 'string') {
      ...
      if (options.headerKeyForProxyUrl) {
        const headerKeyForProxyUrl = options.headerKeyForProxyUrl;
        proxy.on('proxyReq', function(proxyReq, req, res) {
          if (req.headers[headerKeyForProxyUrl]) {
            // url may contain special characters that http headers can't handle
            // so we encode it before sending it to the proxy
            // and at here we decode it back and encode it again to make sure it's encoded correctly
            let newPath = decodeURIComponent(req.headers[headerKeyForProxyUrl]);
            newPath = newPath.split('/').map(s => encodeURIComponent(s)).join('/');
            const proxyUrl = new URL(options.proxy);
            const proxyPath = urlJoin(proxyUrl.pathname, newPath);
            proxyReq.path = proxyPath;
          }
          if (options.proxyLogFn) {
            options.proxyLogFn(proxyReq, req);
          }
        })
      }
      before.push(function (req, res) {
